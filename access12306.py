...
class AccessTrainOrderNetWork:
    ...
    def processLoginCallBack(self, event=None):
        if self.randImage:
            ...
            print(randCode)
            if randCode:
                loginResult = login.login(ht=self.ht, username=self.userInfo[0], password=self.userInfo[1], randCode=randCode, rand=self.login_rand,check_rand_status=self.check_rand_status)
                print(loginResult)
                if loginResult:
                    # 登录成功，关闭登框
                    self.randImage.quit()
